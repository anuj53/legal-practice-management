    ...
    const fetchProfileData = async () => {
      ...
      try {
        const {
          data,
          error
        } = await supabase.from('profiles').select('id, first_name, last_name, email_alias, designation, avatar_url, organization_id').eq('id', user.id).maybeSingle();
        if (error) {
        ...
        if (data) {
          ...
          const {
            data: orgData
          } = await supabase.from('organizations').select('id').eq('name', 'Harvey and Partners').single();
        ...
        <div className="flex items-center gap-3">
          <div className="rounded-full bg-gradient-to-br from-yorpro-400/80 to-yorpro-500/80 flex items-center justify-center shadow-lg border border-white/20 group-hover:scale-105 transition-all duration-300 overflow-hidden h-12 w-12">
            <Avatar className="h-full w-full">
              {profileData?.avatar_url ? <AvatarImage src={profileData.avatar_url} alt={getDisplayName()} className="h-full w-full object-cover" /> : <AvatarFallback className="font-semibold text-white">
                  {getInitials()}
                </AvatarFallback>}
            </Avatar>
          </div>
          {!collapsed && <div className="flex-1 flex items-center justify-between">
              ...
              <Button variant="glass" size="sm" className="w-8 h-8 p-0 flex items-center justify-center hover:bg-white/20 hover:text-red-300" onClick={handleSignOut} title="Sign Out">
